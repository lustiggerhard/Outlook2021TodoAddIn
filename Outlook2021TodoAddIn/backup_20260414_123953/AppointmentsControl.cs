...
namespace Outlook2021TodoAddIn
{
    ...
    public partial class AppointmentsControl : UserControl
    {
        ...
        {
            // SplitterDistance laden wenn Control Höhe bekannt
            if (this.splitContainer1.Height > 50) {
                int sd = Properties.Settings.Default.SplitterDistance;
                if (sd != this.splitContainer1.SplitterDistance &&
                    sd > this.splitContainer1.Panel1MinSize &&
                    sd < this.splitContainer1.Height - this.splitContainer1.Panel2MinSize)
                    this.splitContainer1.SplitterDistance = sd;
            }
            this.apptCalendar.FirstDayOfWeek = this.FirstDayOfWeek;
            this.RetrieveAppointments();
            _listWidth = this.lstAppointments.ClientSize.Width;
            int tileH = this.FontHeight + 26;
            int tileW = this.lstAppointments.ClientSize.Width > 0 ? this.lstAppointments.ClientSize.Width : 300;
            this.lstAppointments.TileSize = new System.Drawing.Size(tileW, tileH);
            if (this.ShowTasks)
            {
                this.RetrieveTasks();
        ...
        private void RetrieveTasks()
        {
            ...
            {
                if (Properties.Settings.Default.Accounts == null || Properties.Settings.Default.Accounts.Count == 0 || Properties.Settings.Default.Accounts.Contains(store.DisplayName))
                {
        ...
        {
            List<OLTaskItem> tasks = new List<OLTaskItem>();
            if (todoFolder == null) return tasks;
        ...
        private void RetrieveAppointments()
        {
            ...
            this.lstAppointments.Scrollable = false;
            typeof(System.Windows.Forms.ListView).GetProperty("DoubleBuffered", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(this.lstTasks, true, null);
            this.lstTasks.Scrollable = false;
            typeof(System.Windows.Forms.ListView).GetProperty("DoubleBuffered", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(this.lstAppointments, true, null);
            System.Windows.Forms.Timer _resizeTimer = new System.Windows.Forms.Timer { Interval = 300 };
            _resizeTimer.Tick += (s, ev) => { _resizeTimer.Stop(); this.RetrieveData(); };
            this.lstAppointments.SizeChanged += (s, ev) => { _resizeTimer.Stop(); _resizeTimer.Start(); };
            this.lstAppointments.Items.AddRange(lstCol.ToArray());
            // Letzten Item entfernen wenn nicht vollständig sichtbar
            while (this.lstAppointments.Items.Count > 0)
            {
                var lastItem = this.lstAppointments.Items[this.lstAppointments.Items.Count - 1];
                var bounds = lastItem.GetBounds(System.Windows.Forms.ItemBoundsPortion.Entire);
                if (bounds.Bottom <= this.lstAppointments.ClientSize.Height) break;
                this.lstAppointments.Items.RemoveAt(this.lstAppointments.Items.Count - 1);
            }

            this.apptCalendar.ShowWeekNumbers = this.ShowWeekNumbers;
            if (!_splitterLoaded && this.splitContainer1.Height > 10) { _splitterLoaded = true; try { this.splitContainer1.SplitterDistance = Properties.Settings.Default.SplitterDistance; } catch { } }
            this.apptCalendar.UpdateCalendar();
            if (_splitterLoaded && this.splitContainer1.Height > 10) { Properties.Settings.Default.SplitterDistance = this.splitContainer1.SplitterDistance; Properties.Settings.Default.Save(); }
            if (this.splitContainer1.Height > 10) { Properties.Settings.Default.SplitterDistance = this.splitContainer1.SplitterDistance; Properties.Settings.Default.Save(); }
        }
        ...
        private void lstTasks_DrawItem(object sender, DrawListViewItemEventArgs e)
        {
            ...
            Rectangle subjectRectangle = totalRectangle; subjectRectangle.Height = this.FontHeight; subjectRectangle.Width = (_listWidth > 0 ? _listWidth : totalRectangle.Width) - 4;
        ...
        private void lstAppointments_DrawItem(object sender, DrawListViewItemEventArgs e)
        {
            ...
            int itemHeight = lineHeight; // Subject
            // Location adds one line
            Outlook.AppointmentItem appt = e.Item.Tag as Outlook.AppointmentItem;
            bool hasLocation2 = !string.IsNullOrEmpty(appt.Location);
            if (hasLocation2) itemHeight += lineHeight;
            e.Graphics.FillRectangle(SystemBrushes.Window, e.Bounds);
            // Übermale den ungenutzten Bereich
            if (!hasLocation2) e.Graphics.FillRectangle(SystemBrushes.Window, new Rectangle(e.Bounds.X, e.Bounds.Y + itemHeight, e.Bounds.Width, e.Bounds.Height - itemHeight));
            ...
            Rectangle statusRectangle = totalRectangle; statusRectangle.Width = horizontalSpacing * 2; statusRectangle.Height = this.FontHeight * 2; statusRectangle.Offset(startRectangleWidth + horizontalSpacing, 0);
            Rectangle subjectRectangle = totalRectangle; subjectRectangle.Height = this.FontHeight; subjectRectangle.Width = (_listWidth > 0 ? _listWidth : totalRectangle.Width) - startRectangleWidth - horizontalSpacing * 4 - 4; subjectRectangle.Offset(startRectangleWidth + horizontalSpacing * 4, 0);
            Rectangle categoriesRectangle = totalRectangle; categoriesRectangle.Width = categoriesRectangleWidth; categoriesRectangle.Height = this.FontHeight; categoriesRectangle.Offset(10, this.FontHeight);
            Rectangle locationRectangle = totalRectangle; locationRectangle.Height = this.FontHeight; locationRectangle.Width = (_listWidth > 0 ? _listWidth : totalRectangle.Width) - startRectangleWidth - horizontalSpacing * 4 - 4; locationRectangle.Offset(startRectangleWidth + horizontalSpacing * 4, this.FontHeight);
            bool selected = e.State.HasFlag(ListViewItemStates.Selected);
            Color back = Color.Empty;
            if (selected) back = Color.LightCyan;
            using (Brush br = new SolidBrush(back))
            ...
            Brush colorBrush = new SolidBrush(this.ForeColor);
            if (!appt.AllDayEvent) e.Graphics.DrawString(appt.Start.ToShortTimeString(), this.Font, colorBrush, startRectangle, rightFormat);

            Color statusColor = catColors.Count > 0 ? catColors[0] : Color.SteelBlue;
            ...
            bool hasLocation = !string.IsNullOrEmpty(appt.Location);
            Rectangle barRect = statusRectangle;
            barRect.Y = totalRectangle.Y;
            barRect.Height = hasLocation ? (this.FontHeight * 2 + 1) : (this.FontHeight + 1);
            Color barColor = catColors.Count > 0 ? catColors[0] : Color.SteelBlue;
            e.Graphics.FillRectangle(new SolidBrush(barColor), barRect);
            if (catColors.Count > 0) {

                Color tint = Color.FromArgb(60, barColor.R, barColor.G, barColor.B);
                int barRight = statusRectangle.Right + 2;
                e.Graphics.FillRectangle(new SolidBrush(tint), new Rectangle(barRight, totalRectangle.Y, totalRectangle.Right - barRight, totalRectangle.Height));
            ...
            if (!hasLocation) e.Graphics.FillRectangle(SystemBrushes.Window, new Rectangle(totalRectangle.X, totalRectangle.Y + this.FontHeight, totalRectangle.Width, totalRectangle.Height - this.FontHeight));
        }
